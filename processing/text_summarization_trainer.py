...
class TextSummTrainer(BaseTrainer):
    ...
    def _resume_checkpoint(self, resume_path):
        super(TextSummTrainer, self)._resume_checkpoint(resume_path)
    ...
    def _train_epoch(self, epoch):
        ...
        for data in self.data_loader:
            ...
            # print(x.shape[0])#max_seq_l
            # print(x.shape[1])#batch
            encoder_outputs = self.model.encoder(x, batch_size=x.shape[1])
            # decoder_hidden = self.model.decoder.init_hidden(x.shape[1])
            decoder_output = self.model.decoder(encoder_outputs, batch_size=x.shape[1])
